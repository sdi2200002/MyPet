...
export default function AppointmentDetails() {
  ...
  const apptId = appId || sp.get("apptId") || "";

  const appt = useMemo(() => {
    const all = safeLoad(APPTS_KEY, []);
    return all.find((a) => String(a.id) === String(apptId)) || null;
  }, [apptId]);

  const vet = useMemo(() => {
    const vets = safeLoad(VETS_KEY, []);
    const vetId = appt?.vetId || appt?.vet?.id || appt?.vet_id;
    if (!vetId) return null;
    return vets.find((v) => String(v.id) === String(vetId)) || null;
  }, [appt]);
  


  const pet = useMemo(() => {
    const pets = safeLoad(PETS_KEY, []);
    const petId = appt?.petId || appt?.pet?.id || appt?.pet_id;
    if (!petId) return null;
    return pets.find((p) => String(p.id) === String(petId)) || null;
  }, [appt]);
  ...
  // ✅ Φωτο κατοικιδίου (στήριξε όλα τα πιθανά keys)
  const petPhoto =
    pet?.photo || appt?.petPhoto || appt?.photoDataUrl || appt?.petImage || appt?.petPhotoUrl || "";


  // ✅ Φωτο κτηνιάτρου (βάλε και τα δικά σου keys αν έχεις)
    ...
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column", bgcolor: "#fff" }}>
      ...
      <Box sx={{ flex: 1 }}>
        <Container maxWidth="lg" sx={{ py: 2.5 }}>
          ...
          >
            {/* Header: title + status pill + buttons */}
            <Stack
            ...
            >
                  ...
                  <Stack direction="row" spacing={2} alignItems="center">
                    ...
                    <Box sx={{ minWidth: 0 }}>
                      ...
                      <Typography sx={{ color: MUTED, fontWeight: 800, mt: 0.3 }}>
                        {vet?.clinicName || appt?.clinicName || appt?.clinicType || "Κλινική μικρών ζώων"}
                      </Typography>
              ...
              {!appt && (
                <Typography sx={{ mt: 2, color: MUTED, fontWeight: 700, fontSize: 12 }}>
                  * Δεν βρέθηκαν στοιχεία ραντεβού (ίσως λάθος id στο URL ή καθάρισες το localStorage).
                </Typography>
              )}
            </Paper>
            {/* <Stack
