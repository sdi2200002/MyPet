...
export default function VetProfile() {
  ...
  const isOwner = isLoggedIn && role === "owner";
  const isVet = isLoggedIn && role === "vet";
  ...
  const onMoreReviews = () => {
    ...
    else navigate(`/vets/${vId}/reviews`); // public
  };
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column", bgcolor: "#fff" }}>
      ...
      <Box sx={{ flex: 1, display: "flex" }}>
        {/* ✅ Sidebar μόνο όταν είναι owner ή vet */}
        {isOwner && <OwnerNavbar />}
        {isVet && <VetNavbar />}

        {/* ✅ main content με offset μόνο όταν υπάρχει sidebar */}
        <Box
          sx={{
            flex: 1,
            display: "flex",
            ...
            ml: {
              xs: 0,
              md: isOwner ? `${OWNER_SIDEBAR_W}px` : isVet ? `${VET_SIDEBAR_W}px` : 0,
            },
          }}
        >
          <Box sx={{ flex: 1 }}>
